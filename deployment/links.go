...
type Link struct {
	...
	AliasedPorts port.PortPairs `json:"AliasedPorts,omitempty"`
	...
	for i := range sources { // each container
		...
		for j := range source.Links { // each link in that container
			...
			for i := range link.Ports {
				if _, ok := link.AliasedPorts.Find(link.Ports[i]); !ok {
					link.AliasedPorts = append(link.AliasedPorts, port.PortPair{link.Ports[i], 0})
				}
			}

			// by default, use all target ports if non-specified
			linkedPorts := link.AliasedPorts
...
func (link containerLink) exposePorts() error {
	...
	for i := range link.AliasedPorts {
		p := link.AliasedPorts[i].Internal
...
func (link containerLink) appendLinks() error {
	...
	for i := range sourceInstances {
		...
		for j := range targetInstances {
			...
			for k := range link.AliasedPorts {
				port := link.AliasedPorts[k]
