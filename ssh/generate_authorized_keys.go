...
func GenerateAuthorizedKeysFor(user *user.User) error {
	...
	for _, handler := range authorizedKeysHandlers {
		if handler.MatchesUser(user) {
			return handler.GenerateAuthorizedKeysFile(user)
		}
	}
...
func (c *containerAuthorizedKeys) GenerateAuthorizedKeysFile(user *user.User) error {
...
// FIXME: Refactor into separate responsibilities for file creation and command
func GenerateAuthorizedKeys(id containers.Identifier, u *user.User, forceCreate bool, printToStdOut bool) error {
	var (
		err      error
		sshKeys  []string
		destFile *os.File
